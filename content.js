...
function execute(){
    ...
    var interval = setInterval(( )=> {
        console.log("HERE",window.webpackChunkwhatsapp_web_client)
...
ExposeStore()
LoadUtils()
console.log(window.Store,"store here")

window.sendMessageToChat =  async function sendMessage(chatId, content, options = {}) {
    let internalOptions = {
...
window.filterContactsByName = async function (name){
        let chats = await window.getChats()
        const matched = chats.filter(chat => chat.formattedTitle.includes(name))
        return matched
}

window.getChats = async function getChats() {
    let chats =  await window.WWebJS.getChats();
    return chats;
}

window.getUnsavedContacts = async function () {
    let chats =  await window.WWebJS.getChats();
    let unsaved = chats.filter(chat => !chat.name)
    return unsaved;
}

window.sendMessageToOpenChat = async function (content) {
   let name = document.getElementsByTagName("header")[1].getElementsByTagName("span")[1].title
   let contact = (await window.filterContactsByName(name))[0]
   console.log(contact)
   window.sendMessageToChat(contact.id._serialized,content)
